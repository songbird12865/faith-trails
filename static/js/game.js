(() => 
{
  // Start the complete game after the page has loaded.
  function init()
  {
  ...
  // Animate the change from one game screen to another.
  ...
  {
    // Draw a curved path through the centers of the quest markers.
  ...
  async function renderCollection(kind)
  {
    ...
    if(kind==='hall')
      {
      // Build one Hall of Fame row for each adventure.
  ...
  {
    // Save the selected difficulty and refresh the trail.
    ...
    // Update the page without restarting the game or its music.
  ...
  // Request a quest and prepare its first scene.
  ...
  function renderQuestShell(){const cover=img(state.quest.slug);questView.innerHTML=`<article class="quest-stage"><img class="quest-art-backdrop" src="${cover}" data-art-src="${cover}" alt="" decoding="async"><img class="quest-art" src="${cover}" data-art-src="${cover}" alt="${esc(state.quest.title)}" decoding="async"><div class="quest-art-shade"></div><div class="scene-panel"><div class="progress-track"><div id="progress-fill" class="progress-fill"></div></div><div id="scene-content" class="scene-content"></div><div class="quest-bottom"><button class="glass-button" id="map-back">← Trail Map</button><div id="scene-controls" class="scene-controls"></div><span class="difficulty-pill">${esc(boot.profile.current_difficulty)}</span></div></div></article>`;questView.querySelector('#map-back').onclick=showMap}
  ...
  // Save the earned badge and check for the Champion reward.
  ...
  // Build the bonus screen for designing a custom badge.
  function renderBadgeDesigner()
  {
    ...
    // Redraw the custom badge after a choice changes.
  ...
  // Animate confetti on the Champion screen.
  ...
  // Animate confetti on the regular badge celebration.
  ...
  // Connect header links and browser history to game screens.
  ...
  // Redraw the trail after the browser window finishes resizing.
  ...
  // Show the trail or open the quest requested by the URL.
  renderMap();if(boot.initialQuest)openQuest(boot.initialQuest,false);
  }
  // Allow the player page to restart the game without a full reload.
